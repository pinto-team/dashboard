...
* WarehousesPage
* ----------------
* Feature page to list, search, paginate and manage warehouse entities.
*/
...
const [page, setPage] = useState<number>(0) // 0-based UI page
    const [pageSize] = useState<number>(12)
        const [query, setQuery] = useState<string>('')
            ...
            const { data, isLoading, error, refetch } = warehousesQueries.useList(listParams)
            ...
            const totalPages = useMemo<number>(
                ...
                const deleteMutation = warehousesQueries.useDelete()
                ...
                const layoutStyle = useMemo<React.CSSProperties>(
                    ...
                    (e: React.ChangeEvent<HTMLInputElement>) => {
                        ...
                        onSuccess: () => {
                        toast.success(t('warehouse.deleted') || t('common.success'))
                        void refetch()
                        },
                        onError: () => {
                                ...
                                <div className="flex flex-1 flex-col gap-4 py-4 md:gap-6 md:py-6">
                                    <div className="flex items-center justify-between px-4 lg:px-6">
                                        ...
                                        <div className="flex gap-2">
                                            <Input
                                                aria-label={t('warehouse.search_placeholder')}
                                                placeholder={t('warehouse.search_placeholder') as string}
                                                value={query}
                                                onChange={handleSearchChange}
                                            />
                                            <Button onClick={handleCreate}>{t('warehouse.create')}</Button>
                                        </div>
                                    </div>
                                    ...
                                    <div className="px-4 lg:px-6">
                                        ...
                                        <WarehousesTable items={items} onDelete={handleDelete} />
                                        )}
                                    </div>
