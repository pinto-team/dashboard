...
import type { CategoryData, CreateCategoryRequest } from '../model/types'
import { categoriesQueries } from '@/features/categories'

function categoryToFormDefaults(e?: CategoryData): Partial<CreateCategoryRequest> {
    if (!e) return {}
    return {
    name: e.name ?? '',
    ...
    React.useEffect(() => {
    if (isLoading) {
    defaultLogger.info('Loading category...', { id })
    return
    }
    if (error) {
    defaultLogger.error('Failed to load category', { id, error: error.message })
    return
    }
    if (data?.data) {
    defaultLogger.info('Category loaded', {
    id: data.data.id,
    name: data.data.name,
    })
    }
    ...
    () => categoryToFormDefaults(data?.data),
    [data],
    )

    const handleDelete = React.useCallback(() => {
    if (!id) return
    if (!window.confirm(t('category.actions.delete'))) return
        ...
        <div className="flex-1 p-6 md:p-8 lg:p-10">
            ...
            <div className="mb-6 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-2xl font-bold">{t('category.edit') ?? t('actions.edit')}</h1>
                </div>
                <div className="flex items-center gap-2">
                    <Button type="submit" form="category-form" disabled={update.isPending}>
            ...
            {
            onSuccess: () => {
            toast.success(t('category.saved_success') ?? t('common.success'))
            navigate(ROUTES.CATEGORY.LIST)
            },
            onError: (err) => {
